...
class Parse(object):
    ...
    # 读取文件
    def read_file(self,filepath: str, mode: Union[str, None] = "rb", encoding: Union[str, None] = None) -> str:
        if encoding:
            content = open(filepath, mode, encoding=encoding).read()
        else:
            content = open(filepath, mode).read()
    ...
    def write_file(self,filepath: str, content: str,mode="w") -> None:
        dir = os.path.dirname(filepath)
        if not os.path.exists(dir):
            os.makedirs(dir)
    ...
    def split_from_str(self,str_all:str)->Union[None,List]:
        str_every_times = str_all.split("***********************************************TextExtractor_OCR.log started***********************************************")
    ...
    def split_from_file(self,filepath:str)->Union[None,List]:
        assert os.path.exists(filepath), print(os.path.abspath(filepath)+"文件不存在")
        str_all = self.read_file(filepath=filepath)
        return self.split_from_str(str_all=str_all)

    # 分析单次日志,输出识别的文件数
    def split_ocr_log(self,log:str,index=None)->Union[None,List]:
        version = re.findall(r"([0-9]{4}-[0-9]{2}-[0-9]{2} [0-9]{2}:[0-9]{2}:[0-9]{2}).*OCR\(DlpOCR\) \$version\$: (.*) on (.*)	, System \$version\$: (\d+.\d+)", log, re.S)
    ...
    def classify_every_file(self,input_ocr_files:List)->Tuple[Union[None,List],Union[None,List],Union[None,List]]:
        # 有效图片 
        ocr_files_valid = [i for i in input_ocr_files if "is not a Picture" not in i]
        ...
        # 识别成功
        ocr_images = [i for i in ocr_files_valid if "OCR Parse Failed" not in i]
        if len(ocr_images)==0:
            print(f"支持识别的图片中全部识别失败")    
        else:
            print(f"支持识别的图片中识别成功图片数: {len(ocr_images)},成功率：{len(ocr_images)/len(ocr_files_valid)}")
        # 求交集即可获得识别失败的图片 
    ...
    def _get_filepath(self,ocr_image:str,filename=None)->str:
        ...
        if filename is not None:
            self.write_file(os.path.join(self.result_dir,filename),path+"\n","a+")
        return path
    ...
    def process_ocr_images(self,ocr_images:List)->Union[None,Tuple[List,List,List]]:
        ...
        for ocr_image in ocr_images:
            ...
            self.write_file(os.path.join(self.dlpocr_dir,filename+".txt"),ocr_result)
    ...
    def task(self,filepath)->Tuple[str,float,str]:
        ocr_infos = []
        # 获取每个日志
        ocr_logs = self.split_from_file("TextExtractor_OCR2.log")
        if ocr_logs is None:
            return None
        for i_log,ocr_log in enumerate(ocr_logs):
            ...
            ocr_images,ocr_files_invalid,ocr_images_fail = self.classify_every_file(input_ocr_files)

            ocr_files_invalid = self.process_files_path(ocr_files_invalid,"ocr_invlid.txt")

            ocr_images_fail = self.process_files_path(ocr_images_fail,"ocr_fail.txt")
...
def main():
    # filepath = "TextExtractor_OCR copy.log"
    filepath = input("请输入日志文件名：")
    print(filepath)
    dlpocr_dir = "dlpocr_dir"
    # 真实值
    label_dir = "dlpocr_label"
    result = "dlpocr_result"
